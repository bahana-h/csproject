...
public class PriceComparator implements java.util.Comparator<TradeOrder> // hannah
{
    ...
    public int compare(TradeOrder order1, TradeOrder order2) {
        // boolean ofr order1
        boolean o1market = order1.isMarket();
        boolean o1limit = order1.isLimit();
        // for order 2
        boolean o2market = order2.isMarket();
        boolean o2limit = order2.isLimit();
        // 0, -1, and 1
        if (o1market == true && o2market == true) {
            ...
            if (ascendingFlag == true) {
                //cents1-2
                return (int)Math.round((price1 - price2)*100);
            } else { // descending
                // cents2-1
                return (int)Math.round((price2 - price1)*100);
            }
        }
    }
}
